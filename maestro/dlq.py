...
def get_sns_arn(topic_name):
	'''
	Gets list of SNS topics & locates the correct one based on name provided in config file

	args:
		topic_name: name of a valid SNS topic
	'''
	list_of_topics = []
	try:
...
def get_sqs_arn(sqs_name):
	'''
	Gets lists of SQS queues & locates the correct one based on name provided in config file

	args:
		sqs_name: name of a valid SQS queue
	'''
	try:
