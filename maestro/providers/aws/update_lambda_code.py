...
def update_funcion_code_zip(lambda_name, archive_name, answer, run):
...
def update_funcion_code_s3(lambda_name, source_bucket, archive_name, answer, run):
    print('Updating function code from %s - %s', source_bucket, archive_name)
    return client.update_function_code(
        FunctionName='%s' % lambda_name,
...
def update_function(lambda_name, source_bucket, archive_name, answer, run):
    if source_bucket is not None:
        upload_deployment_package_to_s3(source_bucket, archive_name)
        return update_funcion_code_s3(lambda_name, source_bucket, archive_name, answer, run)
    else:
        return update_funcion_code_zip(lambda_name, archive_name, answer, run)


def update_code(lambda_name, dry_run=False, publish=False, no_pub=False, source_bucket=None):
    '''
    ...
    if zip_function(lambda_name):
        ...
        try:
            update = update_function(lambda_name, source_bucket, archive_name, answer, run)
